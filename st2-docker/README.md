...
## ChangeLog
### Custom SSL Certs
Create `ssl-certs` directory with the follwing files:
...
Then bind mount it to `/etc/ssl/st2`
  ...
  services:
    stackstorm:
      image: stackstorm/stackstorm:${ST2_IMAGE_TAG:-latest}
       : (snip)
      volumes:
        - ./ssl-certs:/etc/ssl/st2
  ```

### Bind External Driver with `/var/lib/docker/volumes`
Bind a NAS server with `/var/lib/docker/volumes` in `/etc/fstab`, so you can make all data persistance.
